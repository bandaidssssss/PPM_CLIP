...
class TextEncoder_Bayes(nn.Module):

    def __init__(self, clip_model, args):
        super().__init__()
        self.clip_model = clip_model
        self.num_tokens = args.prompt_private_len+args.prompt_share_len
    ...
    def forward(self, visual_feature, prompt_bias_list, mode):  # 移除了text参数
        ...
        if mode == "train":
            x_new_array = torch.zeros((batch_size, visual_feature.shape[0], self.clip_model.text_projection.shape[1]),
            ...
            for i in range(batch_size):
                ...
                visual_feature_new = torch.zeros_like(visual_feature)
                # 应用prompt_bias调整视觉特征
                visual_feature_new[:,:self.prompt_share_len,:]+=prompt_bias_share.reshape(1,1,-1)
                visual_feature_new[:, self.prompt_share_len:self.num_tokens, :] += prompt_bias_private[i].reshape(1, 1, -1) # prompt_bias.shape(bs,embed_dim)
                visual_feature_new[:, self.num_tokens:, :] += prompt_bias_class.reshape(1, 1, -1)
                # 构建输入序列：SOS + 调整后的视觉特征 + EOS和填充
                x_temp = x_pseudo[i].unsqueeze(0).expand(visual_feature.shape[0], -1, -1)
                x_new = torch.cat([
                    x_temp[:, :pos_y, :],               # SOS部分
                    visual_feature_new,                 # 调整后的视觉特征
                    x_temp[:, pos_y + visual_feature_len:, :]  # EOS及后续填充
                ], dim=1)
                # 标准文本编码流程
            ...
            x_new_array = torch.zeros((batch_size, self.prompt_num* self.sample_num,
            ...
            for i in range(batch_size):
                ...
                for j in range(prompt_bias_class.shape[0]):
                    ...
                    visual_feature_new[:, self.prompt_share_len:self.num_tokens, :] += prompt_bias_private[i*self.sample_num+j].reshape(1, 1, -1) # prompt_bias.shape(bs * sample_num,embed_dim)
                    visual_feature_new[:, self.num_tokens:, :] += prompt_bias_class[j].reshape(1, 1, -1) #prompt_bias_state.shape(sample_num,embed_dim)
                    # 构建输入序列
                    x_temp = x_pseudo[i].unsqueeze(0).expand(visual_feature.shape[0], -1, -1)
                    x_new = torch.cat([
                        x_temp[:, :pos_y, :],
...
class TextEncoder_Bayes_V2(nn.Module):
    ...
    def forward(self, visual_feature, prompt_bias_list, mode):  
        ...
        if mode == "train":
            ...
            visual_feature_new = visual_feature_large.clone() # 假设这是你的意图
...
class TextEncoder_Bayes_V3(nn.Module):
    ...
    def forward(self, visual_feature, mode):  # 移除了text参数
        device = visual_feature.device
        dtype = visual_feature.dtype
        embed_dim = visual_feature.shape[-1]
        visual_feature_len = visual_feature.shape[1]
        ...
        if mode == "train":
            ...
            sos = self.clip_model.token_embedding(torch.tensor([[49406]], device=device)).expand(batch_size, -1, -1)
            eos = self.clip_model.token_embedding(torch.tensor([[49407]], device=device)).expand(batch_size, -1, -1)
            x_new = torch.cat([sos, visual_feature, eos], dim=1) 
            current_len = x_new.shape[1]
            padding_len = self.context_length - current_len
            if padding_len > 0:
                padding = torch.zeros(batch_size, padding_len, embed_dim, device=device, dtype=dtype)
                x_new = torch.cat([x_new, padding], dim=1)
            # 标准文本编码流程
            x_new = x_new + self.clip_model.positional_embedding.type(self.dtype)
            x_new = x_new.permute(1, 0, 2)  # NLD -> LND
            ...
            x_new = self.clip_model.ln_final(x_new).type(self.dtype)
            # 提取EOS位置的特征
            x_new_array = x_new[:, eos_index, :] @ self.clip_model.text_projection
            x_new_array=x_new_array.unsqueeze(1)

        else:  # test模式
            batch_size=visual_feature.shape[0]//self.sample_num
            sos = self.clip_model.token_embedding(torch.tensor([[49406]], device=device)).expand(batch_size*self.sample_num, -1, -1)
            eos = self.clip_model.token_embedding(torch.tensor([[49407]], device=device)).expand(batch_size*self.sample_num, -1, -1)

            x_new = torch.cat([sos, visual_feature, eos], dim=1) 
            current_len = x_new.shape[1]
            padding_len = self.context_length - current_len
            if padding_len > 0:
                padding = torch.zeros(batch_size*self.sample_num, padding_len, embed_dim, device=device, dtype=dtype)
                x_new = torch.cat([x_new, padding], dim=1)
            # 标准文本编码流程
            x_new = x_new + self.clip_model.positional_embedding.type(self.dtype)
            x_new = x_new.permute(1, 0, 2)  # NLD -> LND
            ...
            # 提取EOS位置的特征
            x_new_array = x_new[:, eos_index, :] @ self.clip_model.text_projection
            x_new_array=x_new_array.view(batch_size,self.sample_num,-1)
        return x_new_array
...
class Context_Prompting(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.args = args
        self.vision_width = args.vision_width   #1024
        self.text_width = args.text_width   #768
        self.embed_dim = args.embed_dim     #768
    ...
    def forward(self, mode_text_encode, global_img_feature,mode = "train"):
        ...
        # self.zk_private=real_embeddings
        # self.zk_share=fake_embeddings
...
class Context_Prompting_V2(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.args = args
        self.vision_width = args.vision_width   #1024
    ...
    def forward(self, mode_text_encode, global_img_feature,mode = "train"):
        beta = 0
        global_img = global_img_feature
        batch_size=global_img_feature.shape[0]
        ...
        if self.zk_real is None and self.zk_real is None and mode != "train":  # 
            self.zk_real = zk_real
            self.zk_fake = zk_fake
        ...
        if mode == "train":
            ...
            kl_n  = 0
            kl_a = 0
        expanded_zk_private = zk_private.unsqueeze(1).expand(-1, self.args.prompt_private_len, -1)
        expanded_zk_real=zk_real.unsqueeze(1).repeat(batch_size, self.args.prompt_class_len, 1)
        expanded_zk_fake=zk_fake.unsqueeze(1).repeat(batch_size, self.args.prompt_class_len, 1)
        expanded_zk_share=zk_share.unsqueeze(1).repeat(batch_size, self.args.prompt_share_len, 1)

        # expanded_zk_private = zk_private.unsqueeze(1)
        # expanded_zk_real=zk_real.unsqueeze(1).repeat(batch_size, 1, 1)
        # expanded_zk_fake=zk_fake.unsqueeze(1).repeat(batch_size, 1, 1)
        # expanded_zk_share=zk_share.unsqueeze(1).repeat(batch_size, 1, 1)


        context_real= torch.cat([expanded_zk_share,expanded_zk_private , expanded_zk_real], dim = 1)
        context_fake= torch.cat([expanded_zk_share,expanded_zk_private , expanded_zk_fake], dim = 1)
        real_embeddings = mode_text_encode( context_real, mode = mode) 
        fake_embeddings = mode_text_encode(context_fake, mode = mode) 

        text_embeddings = torch.cat([real_embeddings, fake_embeddings], dim =1)
        text_embeddings = text_embeddings / text_embeddings.norm(dim = -1,keepdim = True)
        return text_embeddings, [rec, 0.01*kl ,0.01*kl_n , 0.01*kl_a, 0.01*kl_share]
        # return text_embeddings,[rec,kl]